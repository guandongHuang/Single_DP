...
if not os.path.exists("../img/add_noise_reverse_haar_wavelet/{}/h1".format(name)):
    os.mkdir("../img/add_noise_reverse_haar_wavelet/{}/h1".format(name))
    os.mkdir("../distribution/{}/h1".format(name))
    ...
    for j in range(origin_size // 2):
        ...
        if origin_h1[i][j] > 0:
            ...
            for k in range(201):
                ...
                noise_h1_ = 0.2 * k
                origin_h1_[i][j] += noise_h1_

                # reverse haar wavelet
                img1 = pywt.idwt2((origin_c1_, (origin_h1_, origin_v1_, origin_d1_)), 'haar')
                cv2.imwrite('../img/add_noise_reverse_haar_wavelet/{}/h1/{}_reverse_{}_{}.png'.format(name, name, i, j), img1)

                face_rec.inputPerson(name='musk_face_reverse',
                                     img_path=f'\\..\\img\\add_noise_reverse_haar_wavelet\\{name}\\h1\\{name}_reverse_{i}_{j}.png')
            ...
            for element in range(128):
                np.savetxt('../distribution/{}/h1/sample_minus_mean_{}_{}_{}.txt'.format(name, element, i, j), sample[element])
                plt.cla()
                plt.plot(sample[element], 'bo-', markersize=2)
                plt.xlabel("number")
                plt.ylabel("value")
                plt.savefig("../distribution/{}/h1/{}_{}_{}_{}.png".format(name, element, i, j, mean[element]))