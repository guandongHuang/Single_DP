...
if not os.path.exists("../img/add_noise_reverse_haar_wavelet/{}/v2".format(name)):
    os.mkdir("../img/add_noise_reverse_haar_wavelet/{}/v2".format(name))
    os.mkdir("../distribution/{}/v2".format(name))
    ...
    for j in range(origin_size // 4):
        ...
        if origin_v2[i][j] > 0:
            ...
            for k in range(201):
                ...
                cv2.imwrite('../img/add_noise_reverse_haar_wavelet/{}/v2/{}_reverse_{}_{}.png'.format(name, name, i, j), img1)

                face_rec.inputPerson(name='musk_face_reverse',
                                     img_path=f'\\..\\img\\add_noise_reverse_haar_wavelet\\{name}\\v2\\{name}_reverse_{i}_{j}.png')
            ...
            for element in range(128):
                np.savetxt('../distribution/{}/v2/sample_minus_mean_{}_{}_{}.txt'.format(name, element, i, j), sample[element])
                plt.cla()
                plt.plot(sample[element], 'bo-', markersize=2)
                plt.xlabel("number")
                plt.ylabel("value")
                plt.savefig("../distribution/{}/v2/{}_{}_{}_{}.png".format(name, element, i, j, mean[element]))