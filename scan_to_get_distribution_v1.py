...
if not os.path.exists("../img/add_noise_reverse_haar_wavelet/{}/v1".format(name)):
    os.mkdir("../img/add_noise_reverse_haar_wavelet/{}/v1".format(name))
    os.mkdir("../distribution/{}/v1".format(name))
    ...
    for j in range(origin_size // 2):
        ...
        if origin_v1[i][j] > 0:
            ...
            for k in range(201):
                ...
                noise_v1_ = 0.2 * k
                origin_v1_[i][j] += noise_v1_

                # reverse haar wavelet
                img1 = pywt.idwt2((origin_c1_, (origin_h1_, origin_v1_, origin_d1_)), 'haar')
                cv2.imwrite('../img/add_noise_reverse_haar_wavelet/{}/v1/{}_reverse_{}_{}.png'.format(name, name, i, j), img1)

                face_rec.inputPerson(name='musk_face_reverse',
                                     img_path=f'\\..\\img\\add_noise_reverse_haar_wavelet\\{name}\\v1\\{name}_reverse_{i}_{j}.png')
            ...
            for element in range(128):
                np.savetxt('../distribution/{}/v1/sample_minus_mean_{}_{}_{}.txt'.format(name, element, i, j), sample[element])
                plt.cla()
                plt.plot(sample[element], 'bo-', markersize=2)
                plt.xlabel("number")
                plt.ylabel("value")
                plt.savefig("../distribution/{}/v1/{}_{}_{}_{}.png".format(name, element, i, j, mean[element]))